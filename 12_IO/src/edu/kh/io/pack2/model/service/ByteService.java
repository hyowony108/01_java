...
public class ByteService {
	 ...
	 * 	 모든것을 입/출력 할 수 있음
	 *   (단, 통로가 좁다보니 속도가 좀 느림,
	 * 	 char 형 문자같은 경우 꺠질 우려가 있음)
	 * 
	 */
	
	// IO 관련된 코드 IOExceptoin 발생될 가능성이 높다!
	// -> IO관련 코드는 거의 모든 메서드가 trows IOException
	...
	public void fileByteOutput() {
		...
		try {
			...
			sb.append("~!@#$%^&*()_+<>?\n");
			sb.append("가나다라마바사아\n");
			sb.append("집 가고 싶다 집 집ㅈ ㅣㅂㅈ 빕\n");
			sb.append("선생님 저 공부 정말 열심히 할게요~!\n");
			sb.append("화..화이팅..!\n");
			
			// 출력방법 1 : 한 글자(2byte)씩 파일로 출력
			
			// StringBuilder -> String 변환 -> chatAt -> char로 변환
			...
			*/
			...
			// ******* 수행 시간 확인 *******
			// write() 수행되는 시간 측정
			// System.nanoTime() : 1970.01.01 오전 9 : 00 : 00 qnxj
			...
			fos.flush(); // 스트림 안에 남았는 모든 데이터를 모두 밀어냄
			// FileOutputStream 에서는 flush() 선택사항
	...
	public void bufferedFileByteOutput() {
		 ...
		 * 
		 * - BufferedOutputStream 은 1바이트씩 데이터를 입출력
		...
		try {
			...
			fos = new FileOutputStream("/io_test/20250305/바이트_기반_테스트_Buffered.txt");
			...
			bos.flush(); // 스트림 안에 남아있는 모든 데이트 밀어냄
			// 버퍼 내용을 강제로 파일에 기록
			...
			e.printStackTrace();
 			...
 			try {
 				...
 				// 보조스트림(bos) close() 시
 				// 보조 스트림 생성에 사용된 기반스트림(fos)도 같이 close() 됨
	...
	public void fileByteInput() {
		...
		try {
			...
			// ->  존재하지 않으면  FileNotFountException 발생
			
			// 방법1. 파일 내부 내용을 1byte씩 끊어서 가져오기
			// -> 2byte 범주의 글자들은 깨지는 문제가 발생함
	...
	/**
	 * 외부에 있는 config.propertises 파일 입출력하기
	...
	public void readProperties() {
		...
		try {
			...
			// Properties.getProperty(Key) : key 에 대응되는 value 반환
			System.out.println("username : " + properties.getProperty("username"));
			System.out.println("password : " + properties.getProperty("password"));
			System.out.println("language : " + properties.getProperty("language"));
			//System.out.println("없는키 : " + properties.getProperty("없는키"));
	...
	public void fileCopy() {
		...
		try {
			...
			// 입력용 스트림을 이용해서 target 파일의 내용을 입력받기
			byte[] bytes = bis.readAllBytes();
			
			// ----------------------------------------------------------------
			
			// 출력할 파일 경로 + _copy가 붙은 파일이름
			// target : /io_test/20250304/노래가사.txt
			// copy	  : /io_test/20250304/노래가사_copy.txt
			
			StringBuilder sb = new StringBuilder();
			sb.append(target); // "/io_test/20250304/노래가사.txt"
			
			// int String.lastIndexOf("문자열")
			// - 주어진 문자열이 마지막으로 등장하는 인덱스를 찾아
			// int 값으로 반환. 찾지 못하면 -1반환
