...
// 부모타입 참조변수 = 자식객체
		...
		Car car3 = new Spark();
		// Spark 객체를 참조하는 변수 타입이 Car
		// Stark 객체가 Car 객체로 변화함
		// ->  업 캐스팅
		...
		// Tesla, Spark와 같은ㅇ 자식 객체의 고유 필드, 메서드 사용 불가
		...
		// 상속 + 다형성
		// 상속 특징 : Car 상속 클래스는 모두 getEnginte()을 가지고 있다.
	...
	public void ex3() {
		...
		// 다운 캐스팅이란?
		// 부모 타입 참조변수가 자식 객체를 참조하는 기술(업캐스팅)
		// 적용된 상태에서만 진행할 수 있다.
		...
		// 주의 !
		// "."연산자 (Tesla) 형변환하는 연산자보다 우선 순위가 높음
	...
	public void ex4() {
		...
		Car c1 = new Tesla();
		// Tesla 객체를 만들어 Car 차입 참조변수 c1에 담음
	...
	public void ex5() {
		...
		// c1.toString() 호출코드와
		// String edu.kh.poly.ex1.model.dto.Car.toString () 메소드 코드 연결
		// -> 정적 바인딩
		...
		// -> "실행 시"에는 정적 바인딩보다 자식(Spark)의 메서드와 바인딩
		// == 동적 바인딩
