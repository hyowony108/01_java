...
public class MemberService {
	...
	// 현재 로그인한 한명의 회원의 정보를 저장할 참조 변수 선언
	...
	public int emptyIndex() {
		...
		// memberArr 배열을 0 번째 인덱스부터 끝까지 접근해서
		// 참조하는 값이 null 인 경우 인덱스를 반환
		for(int i = 0; i < memberArr.length; i++) {

			if(memberArr[i] == null) {
				return i;
				// 현재 메서드 종료하고 호출한곳으로 i값 가지고 돌아감
			}
		}
		
		// for문을 수행했지만 return이 되지 않은 경우 해당위치 고드 수행된다
	...
	public void searchRegion() {
		...
		for(int i = 0 ; i < memberArr.length ; i++) {
			...
			//		memberArr[i]요쇼에 저장된 지역(region)이
