...
public class StudentService {
	...
	public void ex() { // List 테스트용 예제
		...
		// Object List.ger(index) : 리스트에서 index번째 인덱스에 있는 객체를 반환
	...
	public void selectAll() {
		...
		// List에 저장된 데이터 개수를 얻어온느 방법 : int List.size();
	...
	/**
	 * 5. 이름이 일치하는 학생을 찾아서 조회하는 메서드(완전일치)
	 * 
	 *  - 검색할 이름 입력받아 studentList에서 꺼내온 
	 *  Student 객체의 name값이 같읁니 비교
	 *  
	 *  - 일치하는 경우 Studetn 객체 출력
	 *  - 일치하는게 없다면 "검색결과가 없습니다" 출력
	 */
	 ...
	 *  List를 정렬하는 방법
	 *  
	 *  방법 1 : Comparable 인터페이스를 상속받아 compareteTo()메서드  재정의
	 *  Student 에 Comparable 인터페이스를 상속받아 오버라이딩 한 compareTo()에
	 *  정의한대로 정렬됨(나이 오름차순, 내림차순,,,)
	 *  
	 *  방법 2 :  Comrarator 클래스에 의한 정렬 compare() 사용 (익명 내부 클래스 이용)
	 *  익병 내부 클래스 : 이름이 없는 클래스를 즉석에서 선언해서 한번만 사용할 목적으로 작성
	 *  객체를 생성하면서 바로 구현내용을 정의할 수 있음
	 *  익명 내부 클래스 장점
	...
	public void SortByName() {
		...
		Collections.sort(studentList,new Comparator<Student>() {
			...
			public int compare(Student o1, Student o2) {
				...
				// (Stirng이 Comparable을 상속받아 재정의해둔 compareTo() 메서드를 이용하는 것)
